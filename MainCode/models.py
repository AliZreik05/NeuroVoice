...
def scaled_sigmoid(x: tf.Tensor) -> tf.Tensor:
    # This function applies a sigmoid activation and then scales the output by 30.
...
class DementiaDetectionModel:
    ...
    def __init__(self, data_file: Union[str, os.PathLike], params_config: dict[str, Any],
        ...
        :param data_file: Dataset file
        :param params_config: 模型参数配置字典
        :param params_config: Model parameter configuration dictionary
        :param model_save_dir: 模型保存路径
        :param model_save_dir: Model saving path
        ...
        if {'audio', 'text', 'handcraft'}.issubset(self.config.keys()):
            assert self.config['audio'] or self.config['text'] or self.config['handcraft'], \
                ...
                # Parameters audio/text/handcraft cannot all be False simultaneously.
        ...
        feat_data = feat_data.sample(frac=1, random_state=rs).reset_index(drop=True)  # 打乱样本 # Shuffle samples
        ...
        # shape=[number of samples, audio sequence length, feature dimension]=[108,7526,39]
        ...
        # shape=(B, T, S)=(108, 1, 7526), where T and H will be broadcast automatically later.
        ...
        self.model_save_dir = os.path.join(model_save_dir, model_name)  # 保存模型路径 # Model saving path
    ...
    def model_create_modal(self):
        """
        构建模型：输入模态选择
        Build Model: Input Modality Selection
        :return: 返回模型
    ...
    def model_create(self):
        ...
        # To ensure audio and text are in the same feature scale space, reduce the audio size using kernel size: o = [i + 2*p - k - (k-1)*(d-1)]/s + 1
        # 这里o=510,i=26843,p=0,d=1,s=1,即输出=(输入-kernel size)/strides+1
        # Here o=510, i=26843, p=0, d=1, s=1, i.e., output=(input-kernel size)/strides+1
        # 公式ref：https://discuss.pytorch.org/t/how-to-keep-the-shape-of-input-and-output-same-when-dilation-conv/14338
        # Formula reference: https://discuss.pytorch.org/t/how-to-keep-the-shape-of-input-and-output-same-when-dilation-conv/14338
        cmha_in_a = Conv1D(filters=128, kernel_size=att_a.shape[1] - att_t.shape[1] + 1, strides=1,
                           padding='valid', use_bias=False)(att_a)  # (,510,128)
        cmha_in_t = Conv1D(filters=128, kernel_size=3, strides=1, padding='same', use_bias=False)(att_t)  # (,510,128)
    ...
    def model_train_evaluate(self, fit: bool = True):
        ...
        with distribute_strategy.scope():
            if fit:
                ...
                if os.path.exists(save_model):  # 存在已训练模型且设置加载， # If a trained model exists and loading is set,
                    print("----------加载模型：{}----------".format(save_model))
                    # ----------Loading model: {}----------
                else:
                    raise FileNotFoundError("模型不存在，无法评估，请先训练")
                    # Model does not exist, cannot evaluate, please train first.
            ...
            if self.config['sig_cls'] or not self.config['sig_reg']:
                ...
                precision, recall, f1_score, support = \
                    precision_recall_fscore_support(yt_label, y_pred_label_ad,
                                                    average='binary', zero_division=1)  # 测试集各项指标 # Test set metrics
...
def model_ablation(data_file: Union[str, os.PathLike], model_dir: Union[str, os.PathLike],
    ...
    :param params_optimal: Optimal model parameter configuration, format is {parameter_name: parameter_value}
    :param params_ablation: 模型消融配置，格式为[(配置名, {参数名: 参数值})]
    :param params_ablation: Model ablation configuration, format is [(config_name, {parameter_name: parameter_value})]
    :param perf_comp_f: 模型性能比较的csv文件
    :param perf_comp_f: CSV file for model performance comparison
    :param fit: 是否进行训练
    :param fit: Whether to train
    :param load_data: 是否加载之前模型已评估好的结果直接获取指标数据
    :param load_data: Whether to load previously evaluated model results to directly get metric data
    :return: 各模型配置的结果
    :return: Results for each model configuration
    """
    if load_data:
...
def compare_with_sota(sota_comp: dict[str, list[dict[str, float]]], sota_comp_dir: Union[str, os.PathLike]):
    ...
    SOTA reference: Shi M, Cheung G, Shahamiri S R. Speech and language processing with deep learning for dementia
        diagnosis: A systematic review. Psychiatry Research, 2023, 329: 115538.
    ...
    :param sota_comp: SOTA results (format: {'Acc': [{'Wang et al., 2022': [93.75]},{'Yuan et al., 2020': [89.58]},
                        {'Liu et al., 2022': [87.50]}], ..., 'RMSE': [{'Koo et al., 2020': 3.74},{'Searle et al., 2020': 4.32},
                        {'Farzana & Parde, 2020': 4.34}]})
    :param sota_comp_dir: 比较结果绘图保存路径
    :param sota_comp_dir: Path to save comparison results plot
    :return: None
    """
...
class Visualization:
    """
    可视化模型内部状态
    Visualize Internal Model States
    """
    ...
    def __init__(self, data_file: Union[str, os.PathLike], fig_save_dir: Union[str, os.PathLike]):
        ...
        :param fig_save_dir: Image saving path
        """
        data_file = os.path.normpath(data_file)
        if data_file.endswith('pkl'):
            feat_data = pd.read_pickle(data_file)  # type: pd.DataFrame
    ...
    def viz_tsne(self, model_file: Union[str, os.PathLike]):
        ...
        Use the output of the ReLU activation layer before the last fully connected layer of the fused model as features,
        perform t-SNE dimensionality reduction and visualize.
        ...
        model_tsne = Model(inputs=model.input, outputs=model.layers[-3].output)  # 最后全连接层之前的ReLU激活层 # ReLU activation layer before the last fully connected layer
    ...
    def viz_feat_imp(self, model_file: Union[str, os.PathLike]):
        """
        可视化手工特征重要性
        Visualize Handcrafted Feature Importance
        :param model_file: 模型文件
        :param model_file: Model file
        :return: None
        """
        ...
        inputs = {"in_a": self.data_audio, "mask_a": self.audio_mask,
                  "in_t": self.data_text, "in_h": self.data_hand}
        model = load_model(model_file, custom_objects=self.custom_objects)
        # Assuming the 'fusion' layer is where handcrafted features are concatenated
        # We need to get the output of the layer that processes only the handcrafted features before concatenation
        # This requires more specific knowledge of the model's architecture to isolate the handcrafted branch's output
        # For a general approach, we can try to inspect the weights related to the handcrafted input in the first Dense layer after fusion.
        # However, directly visualizing feature importance for concatenated features in a complex model like this
        # often requires techniques like SHAP or LIME, which are beyond simple code inspection.
        # For simplicity, if we assume a direct relationship, we can try to look at the weights of the first dense layer that takes 'fusion' as input.

        # Let's try to get the weights of the Dense layer immediately after fusion if it's the `layer_dp`
        # This is a simplification and may not represent true 'feature importance' in a deep, multi-modal model.

        # Find the layer that takes `fusion` as input and is a Dense layer
        dense_layer_after_fusion = None
        for layer in model.layers:
            if isinstance(layer, ConcreteDenseDropout) and layer.name.startswith('concrete_dense_dropout'):
                # This is a heuristic, assuming the first ConcreteDenseDropout after fusion is relevant
                # More robust way: trace connections in model.layers or use model.get_layer() by name if available
                dense_layer_after_fusion = layer.layer # Get the inner Dense layer
                break

        if dense_layer_after_fusion is not None:
            # Get the weights of the Dense layer
            weights = dense_layer_after_fusion.get_weights()[0] # [0] for kernel weights, [1] for bias
            # Assuming the weights are ordered such that the last '14' correspond to handcrafted features
            # This is based on `fusion = Concatenate()([input_at, input_hand])` where `input_hand` is the second input.
            num_handcrafted_features = self.data_hand.shape[-1]
            handcrafted_weights = weights[-num_handcrafted_features:, :]

            # Calculate an "importance" score for each handcrafted feature
            # A simple approach is to take the sum of absolute weights connected to each feature
            feature_importance = np.sum(np.abs(handcrafted_weights), axis=1)

            # Sort features by importance
            sorted_indices = np.argsort(feature_importance)[::-1]
            sorted_importance = feature_importance[sorted_indices]
            sorted_feat_names = [self.hand_feat_name[i] for i in sorted_indices]

            plt.figure(figsize=(10, 7), tight_layout=True)
            sns.barplot(x=sorted_importance, y=sorted_feat_names, palette="viridis")
            plt.xlabel('Importance Score', fontdict={'family': font_family, 'size': 16})
            plt.ylabel('Handcrafted Feature', fontdict={'family': font_family, 'size': 16})
            plt.title('Handcrafted Feature Importance', fontdict={'family': font_family, 'size': 18})
            plt.xticks(fontsize=12, fontproperties=font_family)
            plt.yticks(fontsize=12, fontproperties=font_family)
            for sp in plt.gca().spines:
                plt.gca().spines[sp].set_color('k')
            ...
            fig_file = os.path.join(self.fig_save_dir, f'feat_importance.png')
            if not os.path.exists(os.path.dirname(fig_file)):
                os.makedirs(os.path.dirname(fig_file), exist_ok=True)
            plt.savefig(fig_file, dpi=600, bbox_inches='tight', pad_inches=0.02)
            plt.savefig(fig_file.replace('.png', '.svg'), format='svg', bbox_inches='tight', pad_inches=0.02)
            ...
            print("Could not identify the relevant Dense layer for handcrafted feature importance visualization.")
            # 无法识别用于手工特征重要性可视化的相关密集层。
